...
<div class="gpha-page-shell space-y-5">
    ...
    <section class="gpha-top-pipe rounded-xl border border-slate-200 bg-white p-5 shadow-sm" x-data="{filtersOpen:false}">
        ...
        <form method="GET" action="{{ route('ems.ambulances.show', $ambulance) }}" :class="filtersOpen ? '!grid' : 'hidden'" class="hidden gap-3 md:!grid lg:grid-cols-5">
            ...
            <label class="lg:col-span-2"><span class="gpha-label">Search</span><input name="search" value="{{ request('search') }}" class="gpha-input" placeholder="Reference, location, or purpose"></label>
        ...
        <div class="mt-4 overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table class="gpha-table">
                <thead><tr><th>Reference</th><th>Date</th><th>Route</th><th>Purpose</th><th>Status</th></tr></thead>
                <tbody>
                @forelse($movements as $movement)
                    <tr><td class="font-extrabold text-gpha-primary">{{ $movement->reference }}</td><td>{{ $movement->requested_at?->format('d/m/Y H:i') }}</td><td><span class="font-bold">{{ $movement->origin }}</span><br><span class="text-slate-500">to {{ $movement->destination }}</span></td><td>{{ $movement->purpose }}</td><td><x-ems.status-badge :status="$movement->status" /></td></tr>
                @empty
                    <tr><td colspan="5" class="py-10 text-center text-slate-500">No movements found for the selected filters.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
