...
<main class="sheet">
    ...
    @else
        ...
        <div class="metrics activity-metrics"><div class="metric"><b>{{ $snapshot['total_activities']??0 }}</b>Activity Days</div><div class="metric"><b>{{ $snapshot['follow_ups']??0 }}</b>Follow-ups</div></div>
        <h3 class="band">Daily Operational Activities</h3><div class="table-wrap"><table><thead><tr><th>Date</th><th>Activities, Meetings, Movements and Key Engagements</th></tr></thead><tbody>@forelse($activities as $date=>$rows)@foreach($rows as $row)<tr class="allow-row-split">@if($loop->first)<td class="day" rowspan="{{ $rows->count() }}">{{ Carbon\Carbon::parse($date)->format('l') }}<small>{{ Carbon\Carbon::parse($date)->format('d M Y') }}</small></td>@endif<td><div class="report-rich-text">{!! \App\Support\RichText::clean($row['description']) !!}</div>@if($row['location'])<br><small>Location: {{ $row['location'] }}</small>@endif @if($row['participants'])<br><small>Lead / Participants: {{ $row['participants'] }}</small>@endif @if($row['outcome']??null)<div class="report-rich-text"><b>Outcome:</b> {!! \App\Support\RichText::clean($row['outcome']) !!}</div>@endif</td></tr>@endforeach @empty<tr><td colspan="2" class="empty">No operational movements or departmental activities were recorded for this period.</td></tr>@endforelse</tbody><tfoot class="print-table-footer"><tr><td colspan="2"></td></tr></tfoot></table></div>
    @endif
    ...
    <section class="signatures"><div class="sign prepared"><b>PREPARED BY</b><div><strong class="signer-title">{{ $report->preparedBy?->job_title?:'SAEMT' }}</strong><span class="signer-name">{{ $report->preparedBy?->name??'EMS Report Officer' }}</span>@if($report->submitter_signature_path)<img class="signature-image" src="{{ $reportFileUrl('submitter-signature') }}" alt="Submitter signature">@endif<span class="signature-date">{{ $report->submitted_at?'Submitted':'Prepared' }}: {{ ($report->submitted_at??$report->created_at)?->format('d M Y, H:i') }}</span></div></div><div class="sign"><b>SUBMITTED TO / APPROVED BY</b><div>@if($report->approvedBy||$report->approved_by_name)<strong class="signer-title">{{ $report->approvedBy?->job_title?:'EMS Manager' }}</strong><span class="signer-name">{{ $report->approvedBy?->name??$report->approved_by_name }}</span>@else<strong class="signer-title">EMS Manager</strong><span class="signer-name">Awaiting approval</span>@endif @if($report->approver_signature_path)<img class="signature-image" src="{{ $reportFileUrl('approver-signature') }}" alt="Approver signature">@elseif($report->approver_signature_method==='physical')<span class="physical-note">Physical signature is contained in the uploaded signed PDF.</span>@endif @if($report->approved_at)<span class="signature-date">Approved: {{ $report->approved_at->format('d M Y, H:i') }}</span>@endif</div></div></section>
