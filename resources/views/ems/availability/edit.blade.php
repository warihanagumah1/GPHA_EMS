...
<div class="gpha-page-shell space-y-6">
    ...
    <section class="gpha-panel p-5"><form method="POST" action="{{ route('ems.availability.sessions.update',$session) }}" class="space-y-5">@csrf @method('PUT')
        <div class="grid gap-4 md:grid-cols-3">
            ...
            <label><span class="gpha-label">Session <span class="text-red-600">*</span></span><select name="period" class="gpha-input" required><option value="morning" @selected(old('period',$first->period)==='morning')>Morning</option><option value="afternoon" @selected(old('period',$first->period)==='afternoon')>Afternoon</option><option value="evening" @selected(old('period',$first->period)==='evening')>Evening</option></select></label>
            <x-ems.time-picker name="checked_at" id="availability-checked-at" label="Check Time" :value="old('checked_at',substr($first->checked_at,0,5))" :required="true" />
        </div>
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200"><table class="gpha-table"><thead><tr><th>Department / Unit</th><th>Response <span class="text-red-600">*</span></th><th>Response Location</th><th>Observation</th></tr></thead><tbody>@foreach($checks as $index=>$check)<tr>
            ...
            <td><select name="checks[{{ $index }}][response_location]" class="gpha-input min-w-52"><option value="">Not stated</option>@foreach($locations as $location)<option value="{{ $location }}" @selected(old("checks.$index.response_location",$check->response_location)===$location)>{{ $location }}</option>@endforeach</select></td>
