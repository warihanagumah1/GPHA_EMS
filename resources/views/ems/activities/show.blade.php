...
<div class="gpha-page-shell space-y-6">
    ...
    <section class="gpha-top-pipe rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div><p class="font-extrabold text-gpha-primary">Operations Logs</p><h1 class="text-3xl font-black text-slate-950">Activity Details</h1><p class="mt-2 font-semibold text-slate-500">{{ $activity->activity_date->format('d M Y') }}</p></div>
            <div class="flex flex-wrap gap-2">@if($canManage)<a href="{{ route('ems.activities.edit',$activity) }}" class="gpha-button-secondary">Edit</a><form method="POST" action="{{ route('ems.activities.destroy',$activity) }}" class="inline-flex" data-confirm-title="Delete Activity?" data-confirm-message="Are you sure you want to delete this activity?" data-confirm-label="Yes, Delete Activity" data-confirm-tone="danger">@csrf @method('DELETE')<button class="gpha-button-danger">Delete</button></form>@endif<a href="{{ route('ems.activities') }}" class="gpha-button-primary">Back</a></div>
        </div>
