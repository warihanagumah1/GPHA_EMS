...
<div class="gpha-page-shell space-y-6" x-data="{showForm:@js(request()->boolean('new') || $errors->any())}">
    ...
    <section x-cloak x-show="showForm" x-transition class="gpha-panel p-5">
        ...
        @elseif($module==='availability')
            @php($selectedAvailabilityUnits = collect(old('checks', []))->pluck('unit_name')->filter()->values()->all())
            <form method="POST" action="{{ route('ems.availability.store') }}" class="space-y-5" x-data="{selectedUnits:@js($selectedAvailabilityUnits)}">@csrf
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-200">
                    <table class="gpha-table"><thead><tr><th class="w-24">Include</th><th>Department / Unit</th><th>Response <span class="text-red-600">*</span></th><th>Response Location</th><th>Observation</th></tr></thead>
        ...
        @else
            <form method="POST" action="{{ route('ems.activities.store') }}" class="space-y-5" x-data="{followUp:@js(old('requires_follow_up')==='1'),outcomeOpen:@js(filled(old('outcome')))}">@csrf
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2"><span class="gpha-label">Activities for the Day <span class="text-red-600">*</span></span><x-ems.rich-text-editor name="description" :value="old('description')" placeholder="Use a bulleted or numbered list to record the different activities completed on this date" required /></div>
    ...
    <section class="gpha-panel overflow-hidden">
        ...
        <div class="overflow-x-auto"><table class="gpha-table"><thead><tr>
            ...
            @elseif($module==='dispatches')<th>Reference</th><th>Date</th><th>Ambulance</th><th>Route</th><th>Priority</th><th>Status</th>@if($canManage)<th class="gpha-actions-heading">Actions</th>@endif
            ...
            @else<th>Date</th><th>Activities</th><th>Outcome</th><th>Follow-up</th><th class="gpha-actions-heading">Actions</th>@endif
        </tr></thead><tbody>
            ...
            @forelse($rows as $row)<tr>
                ...
                @elseif($module==='availability')
                    ...
                    <td class="gpha-actions-cell"><div class="relative inline-block text-left" x-data="{open:false,menuTop:0,menuLeft:0,positionMenu(){const r=this.$refs.trigger.getBoundingClientRect(),w=224,h=170,p=8;this.menuTop=Math.max(p,Math.min(r.top,window.innerHeight-h-p));this.menuLeft=r.right+p+w<=window.innerWidth-p?r.right+p:Math.max(p,r.left-w-p)}}" @resize.window="open&&positionMenu()" @scroll.window="open&&positionMenu()" @keydown.escape.window="open=false"><x-ems.action-trigger x-ref="trigger" x-bind:class="{'is-open':open}" @click.stop="positionMenu();open=!open" label="Check session actions" /><div x-cloak x-show="open" x-transition @click.outside="open=false" :style="`top:${menuTop}px;left:${menuLeft}px`" class="gpha-floating-action-menu"><a href="{{ route('ems.availability.sessions.show',$row->session_uuid) }}" class="block w-full px-4 py-2 text-left font-semibold text-emerald-700 hover:bg-emerald-50">View</a>@if($canManage)<a href="{{ route('ems.availability.sessions.edit',$row->session_uuid) }}" class="block w-full px-4 py-2 text-left font-semibold text-gpha-primary hover:bg-blue-50">Edit</a><form method="POST" action="{{ route('ems.availability.sessions.destroy',$row->session_uuid) }}" data-confirm-title="Delete Check Session?" data-confirm-message="Are you sure you want to delete this check session?" data-confirm-label="Yes, Delete Session" data-confirm-tone="danger">@csrf @method('DELETE')<button type="submit" @click="open=false" class="block min-h-0 w-full px-4 py-2 text-left font-semibold text-red-600 hover:bg-red-50">Delete</button></form>@endif</div></div></td>
                @else<td>{{ $row->activity_date->format('d M Y') }}</td><td><p class="max-w-xl font-semibold text-slate-700">{{ str(\App\Support\RichText::plain($row->description))->limit(150) }}</p></td><td>{{ $row->outcome?str(\App\Support\RichText::plain($row->outcome))->limit(80):'—' }}</td><td>@if($row->requires_follow_up)<span class="font-bold text-amber-700">{{ $row->follow_up_action }}<br><small>{{ $row->follow_up_owner }}{{ $row->follow_up_due_date?' · due '.$row->follow_up_due_date->format('d M Y'):'' }}</small></span>@else<span class="font-bold text-emerald-700">Closed</span>@endif</td><td class="gpha-actions-cell"><div class="relative inline-block text-left" x-data="{open:false,menuTop:0,menuLeft:0,positionMenu(){const r=this.$refs.trigger.getBoundingClientRect(),w=224,h=170,p=8;this.menuTop=Math.max(p,Math.min(r.top,window.innerHeight-h-p));this.menuLeft=r.right+p+w<=window.innerWidth-p?r.right+p:Math.max(p,r.left-w-p)}}" @resize.window="open&&positionMenu()" @scroll.window="open&&positionMenu()" @keydown.escape.window="open=false"><x-ems.action-trigger x-ref="trigger" x-bind:class="{'is-open':open}" @click.stop="positionMenu();open=!open" label="Activity actions" /><div x-cloak x-show="open" x-transition @click.outside="open=false" :style="`top:${menuTop}px;left:${menuLeft}px`" class="gpha-floating-action-menu"><a href="{{ route('ems.activities.show',$row) }}" class="block w-full px-4 py-2 text-left font-semibold text-emerald-700 hover:bg-emerald-50">View</a>@if($canManage)<a href="{{ route('ems.activities.edit',$row) }}" class="block w-full px-4 py-2 text-left font-semibold text-gpha-primary hover:bg-blue-50">Edit</a><form method="POST" action="{{ route('ems.activities.destroy',$row) }}" data-confirm-title="Delete Activity?" data-confirm-message="Are you sure you want to delete this activity?" data-confirm-label="Yes, Delete Activity" data-confirm-tone="danger">@csrf @method('DELETE')<button type="submit" @click="open=false" class="block min-h-0 w-full px-4 py-2 text-left font-semibold text-red-600 hover:bg-red-50">Delete</button></form>@endif</div></div></td>
                @endif
