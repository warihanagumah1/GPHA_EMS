...
@php
    ...
    $savedDestination = old('destination', $movement?->destination);
    $destinationChoice = in_array($savedDestination, $locations, true) || $savedDestination === 'Other' ? $savedDestination : (filled($savedDestination) ? 'Other' : '');
    $otherDestination = old('destination_other', $destinationChoice === 'Other' && $savedDestination !== 'Other' ? $savedDestination : '');
@endphp
...
<form method="POST" action="{{ $action ?: route('ems.dispatches.store') }}" class="space-y-5">
    ...
    <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-5" x-data="{notesOpen:@js(filled(old('notes',$movement?->notes))),status:@js(old('status',$movement?->status ?? 'requested'))}">
        <div class="grid gap-x-5 gap-y-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div><span class="gpha-label">Origin <span class="text-red-600">*</span></span><x-ems.searchable-select name="origin" :options="[...$locations,'Other']" :value="$originChoice" placeholder="Search and select origin" other-name="origin_other" :other-value="$otherOrigin" other-label="Other Origin" other-placeholder="Enter the origin" /></div>
            <div><span class="gpha-label">Destination <span class="text-red-600">*</span></span><x-ems.searchable-select name="destination" :options="[...$locations,'Other']" :value="$destinationChoice" placeholder="Search and select destination" other-name="destination_other" :other-value="$otherDestination" other-label="Other Destination" other-placeholder="Enter the destination" /></div>
