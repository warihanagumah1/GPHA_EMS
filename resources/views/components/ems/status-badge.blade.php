...
@php
    ...
    $label = config("ems.movement_priorities.{$status}") ?? str($status)->replace('_', ' ')->lower()->title();
    $classes = match ($value) {
        ...
        'NON_EMERGENCY', 'URGENT', 'MAINTENANCE' => 'bg-amber-50 text-amber-700 ring-amber-200',
        'EMERGENCY', 'CRITICAL', 'UNAVAILABLE', 'CANCELLED', 'NEGATIVE' => 'bg-red-50 text-red-700 ring-red-200',
