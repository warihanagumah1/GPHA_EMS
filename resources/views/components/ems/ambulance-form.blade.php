@props(['ambulance' => null, 'locations', 'action', 'method' => 'POST', 'submitLabel' => 'Save Ambulance'])
...
<form method="POST" action="{{ $action }}" class="space-y-5">
    ...
    <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-5">
        ...
        <div class="grid gap-x-5 gap-y-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <label x-data="{ registration: @js(old('registration_number', $ambulance?->registration_number ?? '')), registrationTouched: false, normalizeRegistration(value) { return value.trim().replace(/\s+/g, ' ').toUpperCase() }, registrationValid() { return /^[A-Z]{1,3} [0-9]{1,4}-[0-9]{2}$/.test(this.normalizeRegistration(this.registration)) } }">
                <span class="gpha-label">Registration No. <span class="text-red-600">*</span></span>
                <input name="registration_number" x-model="registration" @input="registrationTouched = true" @blur="registration = normalizeRegistration(registration); $el.value = registration; registrationTouched = true" :aria-invalid="registrationTouched && !registrationValid()" aria-describedby="registration-format-feedback" class="gpha-input uppercase" :class="{'!border-red-500 !ring-red-200': registrationTouched && !registrationValid(), '!border-emerald-500 !ring-emerald-200': registrationTouched && registrationValid()}" placeholder="E.g. GV 1234-26" pattern="[A-Za-z]{1,3} [0-9]{1,4}-[0-9]{2}" title="Use the Ghana format GV 1234-26" autocomplete="off" maxlength="30" required>
                <span id="registration-format-feedback" class="gpha-help-text mt-1 block">
                    <span x-show="!registrationTouched" class="text-slate-500">Format: GV 1234-26</span>
                    <span x-cloak x-show="registrationTouched && !registrationValid()" role="alert" class="font-bold text-red-600">Enter a valid registration number such as GV 1234-26.</span>
                    <span x-cloak x-show="registrationTouched && registrationValid()" role="status" class="font-bold text-emerald-700">Valid registration format.</span>
                </span>
            </label>
            ...
            <label><span class="gpha-label">Base Location <span class="text-red-600">*</span></span><select name="base_location" class="gpha-input" required><option value="">Select base location</option>@foreach($locations as $location)<option value="{{ $location }}" @selected(old('base_location',$ambulance?->base_location)===$location)>{{ $location }}</option>@endforeach</select></label>
